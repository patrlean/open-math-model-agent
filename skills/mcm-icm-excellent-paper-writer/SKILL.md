...
- 单张图片通常设为正文行宽的 `0.6–0.8` 倍，即使用 `width=0.6\textwidth` 至 `width=0.8\textwidth`；仅当图内信息密集、缩小后无法辨认时才适当放大，不要习惯性铺满整行。
- 两张需要直接比较或共同说明一个结论的图片优先并排，每张图片或子图容器设为约 `0.45\textwidth`，保留适当水平间距并对齐；在 `minipage` 或 `subfigure` 内部使用 `width=\linewidth` 填满各自容器。
- 并排后若坐标轴、图例或标注无法清晰阅读，则改为上下排列，不得为了节省页数牺牲可读性。
...
本 Skill 当前采用的页数档案是：官方上限 25 页，写作目标 25 页，24–25 页可以验收。若当年官方规则发生变化，必须同时更新本文件开头的 `paper` 配置和本节说明。
...
在第一次调用 `write_paper` 前，必须调用：

```text
load_skill_file(
  skill='mcm-icm-excellent-paper-writer',
  path='references/latex-writing-guide.md'
)
```

完整遵守其中的转义、数学模式、结果引用、静态检查、错误分类、局部修订和停止条件。尤其要做到：

- 对最终 Summary 精确计数，不凭感觉估算字数；
- 每个正文一级章节使用独立的 `sections` 数组项；章节数量和英文标题按模型叙事自由决定，附录与参考文献分别使用专用字段；
- 编译前排查文本模式中的 `_`、`^` 和不成对的 `$`；
- 首次编译成功后，局部缺陷只用 `inspect_paper_blocks` 与 `edit_paragraph` 修复；
- 只处理工具明确列出的失败项，不自行提高验收阈值；
- 一旦 `paper acceptance PASSED`，不得仅为从 24 页凑到 25 页而继续改稿。
